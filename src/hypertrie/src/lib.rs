...
pub unsafe extern "C" fn trie_insert(trie: *mut Trie, word: *const u8, len: usize) {
    ...
    unsafe {
        let slice = slice::from_raw_parts(word, len);
        (*trie).insert(slice);
    }
}
...
pub unsafe extern "C" fn trie_words_with_prefix(
    ...
    unsafe {
        let slice = slice::from_raw_parts(prefix, prefix_len);
        let words = (*trie).words_with_prefix(slice);
...
pub unsafe extern "C" fn trie_bulk_insert(
    ...
    unsafe {
        let words_slice = slice::from_raw_parts(words, len);
        let lens_slice = slice::from_raw_parts(word_lens, len);
        let trie = &mut *trie;
        for i in 0..len {
            let word_ptr = words_slice[i];
            let word_len = lens_slice[i];
            if !word_ptr.is_null() {
                let slice = slice::from_raw_parts(word_ptr, word_len);
                trie.insert(slice);
            }
        }
    }
